...
fn build_ui(app: &Application) {
    ...
    let header_box = create_header_section(&sys);
    main_box.append(&header_box);
    
    main_box.append(&Separator::new(Orientation::Horizontal));

    // System overview section (CPU, RAM, GPU)
    let overview_box = create_overview_section(&sys);
    main_box.append(&overview_box);
    
    main_box.append(&Separator::new(Orientation::Horizontal));

    // Process list section
    let process_scroll = create_process_list(&sys);
    main_box.append(&process_scroll);
    
    main_box.append(&Separator::new(Orientation::Horizontal));

    // Disk usage section
    let disk_box = create_disk_section(&sys);
            ...
            for i in 0..device_count {
                if let Ok(device) = nvml.device_by_index(i) {
                    if let Ok(name) = device.name() {
                        ...
                        // Add utilization
                        if let Ok(util) = device.utilization_rates() {
                            info.push_str(&format!(" (GPU: {}%", util.gpu));
                            if let Ok(temp) = device.temperature(nvml_wrapper::enum_wrappers::device::TemperatureSensor::Gpu) {
                                info.push_str(&format!(", {}°C", temp));
                            }
                            info.push(')');
        ...
        if let Ok(entries) = fs::read_dir("/sys/class/drm") {
            for entry in entries.flatten() {
                ...
                if let Some(name) = path.file_name() {
                    let name_str = name.to_string_lossy();
                    if name_str.starts_with("card") && !name_str.contains('-') {
                        // Try to read GPU name from device
                        let device_path = path.join("device/vendor");
                        if device_path.exists() {
                            if let Ok(vendor) = fs::read_to_string(&device_path) {
                                let vendor = vendor.trim();
...
fn create_header_section(sys: &Rc<RefCell<System>>) -> Box {
    ...
    cpu_label.set_markup(&format!(
        "<span size='x-large' weight='bold'>🖥️  {}</span>",
        cpu_model
    ));
    cpu_label.set_xalign(0.0);
    ...
    for gpu in gpus {
        let gpu_label = Label::new(None);
        gpu_label.set_markup(&format!(
            "<span size='large' weight='bold'>🎮  {}</span>",
            gpu
        ));
        gpu_label.set_xalign(0.0);
        header_box.append(&gpu_label);
    }
    
    header_box
}

fn create_overview_section(sys: &Rc<RefCell<System>>) -> Box {
    let overview_box = Box::new(Orientation::Horizontal, 30);
    ...
    let cpu_label = Label::new(None);
    cpu_label.set_markup(&format!(
        "<span size='large' weight='bold'>🔥 CPU Usage\n</span>\
        <span size='x-large' weight='bold'>{:.1}%</span>\n\
        <span size='small'>{} cores</span>",
        cpu_usage, cpu_cores
    ));
    ...
    let ram_label = Label::new(None);
    ram_label.set_markup(&format!(
        "<span size='large' weight='bold'>💾 Memory\n</span>\
        <span size='x-large' weight='bold'>{:.1}%</span>\n\
        <span size='small'>{:.0} MB / {:.0} MB</span>",
        mem_percent, used_mem, total_mem
    ));
    
    // Swap usage
    let total_swap = sys_ref.total_swap() as f64 / 1024.0 / 1024.0;
    let used_swap = sys_ref.used_swap() as f64 / 1024.0 / 1024.0;
    ...
    swap_label.set_markup(&format!(
        "<span size='large' weight='bold'>💿 Swap\n</span>\
        <span size='x-large' weight='bold'>{:.1}%</span>\n\
        <span size='small'>{:.0} MB / {:.0} MB</span>",
        swap_percent, used_swap, total_swap
    ));
    
    overview_box.append(&cpu_label);
    overview_box.append(&ram_label);
    overview_box.append(&swap_label);
    
    overview_box
}

fn create_process_list(sys: &Rc<RefCell<System>>) -> ScrolledWindow {
    let scroll = ScrolledWindow::builder()
        .hscrollbar_policy(gtk::PolicyType::Never)
    ...
    let name_label = Label::new(None);
    name_label.set_markup("<b>Process Name</b>");
    name_label.set_width_chars(35);
    name_label.set_xalign(0.0);
    ...
    // Process rows (top 25 by CPU usage)
    let mut processes: Vec<_> = sys_ref.processes().iter().collect();
    processes.sort_by(|a, b| {
        b.1.cpu_usage().partial_cmp(&a.1.cpu_usage()).unwrap()
    });
...
fn create_disk_section(sys: &Rc<RefCell<System>>) -> Box {
    let disk_box = Box::new(Orientation::Vertical, 5);
    
    let header = Label::new(None);
    header.set_markup("<span size='large' weight='bold'>💽 Disk Usage</span>");
    header.set_xalign(0.0);
    disk_box.append(&header);
    ...
    for disk in sys_ref.disks() {
        ...
        let disk_label = Label::new(None);
        disk_label.set_markup(&format!(
            "<span font_family='monospace'>{}: {:.1} GB / {:.1} GB ({:.1}% used)</span>",
            disk.mount_point().display(),
            used_gb,
            total_gb,
            used_percent
